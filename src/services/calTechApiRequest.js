...
function getSelectQueryString(select) {
	let queryString = ""; // set prefix query string with distinct which makes sure that only selected columns are returned
	let checkedCounter = 0; // counter for checked checkboxes
	...
	Object.entries(select).forEach(([columnName, checked]) => {
		if (checked) {
			...
			checkedCounter === 0 ? queryString += `${columnName}` : queryString += `,${columnName}`;
			checkedCounter += 1;
		}
	});
