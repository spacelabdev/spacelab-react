...
export default function CohortsSlider({ toggle }) {
	...
	// Member card data
	const { title: december2021Title, members: december2021Members } =
		december2021Cohort;
	const {
	...
	return (
		<div id={"cohort-component-container"} className={toggle}>
			<SliderNav
				...
				activeSlide={activeCohort}
				setActive={setActiveCohort}
			/>
			<SliderComponent
				active={`${activeCohort === "december2021" ? "" : "hidden"}`}
				title={december2021Title}
				members={december2021Members}
			/>
			<SliderComponent
				active={`${
					activeCohort === "additionalContributors" ? "" : "hidden"
				}`}
				title={additionalContributorsTitle}
				members={additionalContributorMembers}
			/>
