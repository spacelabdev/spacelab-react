	...
	useEffect(() => {
		if (exoPlanetData && exoPlanetData[0]) {
			...
			if (!columnHeaders) {
				...
				const hashTable = {};
				Object.keys(exoPlanetData[0]).forEach(
					(colName, idx) => (hashTable[colName] = true)
				);
				...
				const newColumnHeaders = columnHeaders.filter((colName) => {
					const isColNameInHashTable =
						hashTable.hasOwnProperty(colName);
				...
				Object.keys(hashTable).forEach((colName) =>
					newColumnHeaders.push(colName)
				);
			...
			setExoPlanetData((prevState) => {
				...
				if (!sortOrder) {
					...
					return newState.sort((firstRow, secondRow) => {
						...
						const colDataItem = aggregateDataItems.filter(
							(col) => col.name === sortColName
						);
